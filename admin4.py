...
def add_upstox_to_mongo(admin_id, account_number, api_key):
    
    """Stores Upstox broker details in MongoDB."""
    broker_data = {
        "admin_id": admin_id,
        "broker": "Upstox",
    ...
    db = client["user_database"]
    collection = db["users"]
    collection.insert_one(broker_data)
 
def add_upstox_function(admin):
    """Displays the Upstox form and saves data to MongoDB on submission."""
    st.subheader("Upstox Broker Form")
    
    with st.form("upstox_form"):
        account_number = st.text_input("Upstox Account Number")
        api_key = st.text_input("API Key", type="password")
        submitted = st.form_submit_button("Submit")
        
        if submitted:
            if account_number and api_key:
                add_upstox_to_mongo(admin["_id"], account_number, api_key)
                st.success("Upstox details saved successfully!")
            else:
                st.error("Please fill in all fields.")

def add_angel_one_function(admin):
    st.subheader("Angel One Broker Form")
    with st.form("angel_one_form"):
                ...
                for broker in broker_list:
                    ...
                    with col_b2:
                        if st.button("Add Broker", key=f"add_broker_{admin['_id']}_{broker}"):
                            function_name = f"add_{broker.lower().replace(' ', '_')}_function"
                            if function_name in globals():
                                globals()[function_name](admin)  # Call the function dynamically
                            else:
                                st.error(f"Function {function_name} not found!")  # Handle missing function case
